...
import {Button, Card, CardActions, CardContent, CardMedia, Grid, Pagination, Typography, Box} from "@mui/material";
import axios from "axios";

const NewsPage = () => {
    const [news, setNews] = useState({articles: [], totalResults: 0});
    const [pagination, setPagination] = useState({page: 1, count: 1});
    const [searchParam, setSearchParam] = useState("ukraine");
        ...
        <>
            ...
            <Box sx={{display: "flex", justifyContent: "center", gap: 2, marginTop: "20px"}}>
                {topics.map((topic) => (
                    <Button
                        key={topic.value}
                        variant={searchParam === topic.value ? "contained" : "outlined"}
                        onClick={() => changeTopicHandler(topic.value)}
                    >
                        {topic.label}
                    </Button>
                ))}
